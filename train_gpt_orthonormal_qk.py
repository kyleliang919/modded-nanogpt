...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, ve: Tensor | None, lambdas: Tensor, block_mask: BlockMask):
        ...
        k_proj = self.qkv_w[1]
        v_proj = self.qkv_w[2]
        qk_proj = zeropower_via_newtonschulz5(q_proj.transpose(0, 1) @ k_proj, steps = 5)
        norm_x = norm(x)
        q = F.linear(norm_x, qk_proj.type_as(x)).view(B, T, self.num_heads, self.head_dim)
        k = norm_x
        v = F.linear(x, v_proj.type_as(x)).view(B, T, self.num_heads, self.head_dim)
